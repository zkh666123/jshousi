 ...
 * 重写 some
 *
 * Array.prototype.some(callback, thisArg) 的规则：
 * 1. 只要有一个元素满足 callback，就返回 true。
 * 2. 所有已存在元素都不满足 callback，返回 false。
 * 3. 空数组返回 false。
 * 4. 稀疏数组空位会被跳过。
 */
 ...
 * @return {boolean}
 */
function some2(arr, callback, thisArg) {
  for (let i = 0; i < arr.length; i++) {
