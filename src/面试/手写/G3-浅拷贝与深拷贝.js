 ...
 * 深拷贝的实现原理：遍历对象，然后把获取属性和属性值并放到一个新对象中，但是在拷贝的时候需要判断属性值的类型是值类型还是引用类型，如果是引用类型需要递归调用深拷贝函数。
 *
 * 继承属性和不可枚举属性是不能拷贝的
 *
 * 浅拷贝的几种方式
 ...
 * 我们只有手动将 obj = null，它才会被垃圾回收机制进行回收，
  ...
  if (syskeys.length) {
    syskeys.forEach((syskey) => {
      if (typeof target[syskey] === "object" && target[syskey] !== null) {
        cloneTarget[syskey] = deepClone1(target[syskey], hash);
      } else {
...
let obj = {
  ...
  g: function () {
    console.log('clone function', this.c)
  },
  h: Symbol("h"),
  i: { ii: new Date() },
};
