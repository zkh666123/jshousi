 ...
 */
function eachFlat3(arr, depth) {
  const result = [];
  arr.forEach((item) => {
    if (Array.isArray(item) && depth > 0) {
      result.push(...eachFlat3(item, depth - 1));
    } else {
      result.push(item);
    }
  });
...
const flat33 = eachFlat3(arr, Infinity);
console.log("方法 ->", flat33);
...
const flat5 = mapFlat(arr, Infinity);
