 ...
 * 3. 窗口大小 resize 事件的触发。只需窗口调整完成后，计算窗口大小。防止重复渲染。调整浏览器窗口大小时，resize 次数过于频繁，造成计算过多，此时需要一次到位，就用到了防抖
 * 4. 鼠标的 mousemove、mouseover 事件的触发。
 * 5. 导航条上，用户不停的在导航区域滑动
 * 6. 登录、发短信等按钮提交场景，防止多次提交按钮，以致于发送了多次请求，只执行最后提交的一次，需要防抖
 * 7. 文本编辑器实时保存，当无任何更改操作一秒后进行保存
 *
 ...
 * 5. input 框实时搜索并发送请求展示下拉列表，每隔一秒发送一次请求 (也可做防抖)
