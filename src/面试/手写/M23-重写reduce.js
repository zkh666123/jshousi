...
function depthFlat(arr, depth = 1) {
  // 用 reduce 累积结果，避免外部变量
  // 注意 depth - 1 而不是 --depth，否则 closure 中的 depth 会被兄弟节点共享递减
  return arr.reduce((acc, val) => {
    if (Array.isArray(val) && depth > 0) {
      return acc.concat(depthFlat(val, depth - 1));
    }
...
runPromiseInSequence(promiseArr, 10).then((value) => {
  console.log("重写reduce,按顺序运行Promise ->", value);
});
